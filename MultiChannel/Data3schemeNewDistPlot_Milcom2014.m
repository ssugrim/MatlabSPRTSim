...
plot(Channels,Data_SPRT_channels_found_H0,'-^g',Channels,Data_Tree_channels_found_H0,'-*r',Channels,Data_Simple_channels_found_H0,'-ob',x,x .* .4,'-xm');
legend('greedy-SPRT, \lambda = 0.3', 'Tree, \psi = 4', 'Simple','Actual number of consistent channels');
xlim([20,1050]);
xlabel('N, number of Channels');
ylabel('Number of channels classifed as H_{C,n}');
set(findall(f,'type','text'),'fontSize',20,'fontWeight','bold','LineWidth',10);


f = figure(2);
plot(Channels,Data_SPRT_error_2,'-^g',Channels,Data_Tree_error_2,'-*r',Channels,Data_Simple_error_2,'-ob');
legend('greedy-SPRT, \lambda = 0.3', 'Tree, \psi = 4', 'Simple');
xlim([20,1050]);
xlabel('N, number of Channels');
ylabel('Probablity of missclasification as H_{C,n}');
set(findall(f,'type','text'),'fontSize',20,'fontWeight','bold','LineWidth',10);
% 
% f = figure(3);
...
legend('greedy-SPRT, \lambda = 0.3', 'Tree, \psi = 4');
xlim([20,1050]);
xlabel('N, number of Channels');
ylabel('Number of channels classifed as H_{C,n}');
set(findall(f,'type','text'),'fontSize',20,'fontWeight','bold','LineWidth',4);
