...
Data_good_available = zeros(length(lambda_range),Trials,length(Channels));
...
Data_SPRT_percent_classified = zeros(length(lambda_range),Trials,length(Channels));
...
for lambda_ind  = 1:length(lambda_range)
    ...
    %Swapping out the K loop for a single value.
    for chan_num_ind = 1:length(Channels)
        ...
        parfor trial_ind = 1:Trials
            ...
            Data_good_available(lambda_ind,trial_ind,chan_num_ind) = sum(p_act < p_prime_ls);
            
            
            % Use the bins to compute the error rates
            [Data_SPRT_error_1(lambda_ind,trial_ind,chan_num_ind),Data_SPRT_error_2(lambda_ind,trial_ind,chan_num_ind)] = TrialAlphaBeta(K,bin_p_act,Trial_SPRT_decision);
            ...
            % Percent Classified by SPRT
            Data_SPRT_percent_classified(lambda_ind,trial_ind,chan_num_ind) = Data_SPRT_Significant(lambda_ind,trial_ind,chan_num_ind) / K;
...
matlabpool close;
fname = sprintf('DataMultiSPRTTrials%dLambdas%d.mat',Trials,length(lambda_range));
save(fname);
