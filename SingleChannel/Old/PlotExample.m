...
for k = 1:1:K
    ...
    legendInfo2{k} = sprintf('C_%d, p_{act} = %2.2f',k,p_act(k));
end
...
legend(legendInfo2);
titlestr = sprintf('Cost per channel, lambda = %2.2f ',lambda);
title(titlestr);
xlabel('Time Index (m)');
ylabel('Cost');
hold off;
...
for k = 1:1:K
    ...
    legendInfo3{k} = sprintf('M_%d = %d , p_{n} = %2.2f',k,decided_at(k),p_act(k));
end
axis([0,max_rnd,0,round(max(max(clocks(:,1:max_rnd)))+10)]);
legend(legendInfo3);
titlestr = sprintf('Measurements per channel, lambda = %2.2f ',lambda);
title(titlestr);
